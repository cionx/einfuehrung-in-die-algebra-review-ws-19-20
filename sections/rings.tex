...
\begin{definition}
  Eine Teilmenge~$I$ von~$R$ ist ein \defemph{Ideal}, falls die folgenden beiden Bedingungen gelten:
  \begin{enumerate}
    \item
...
\begin{example}
  ...
  \begin{enumerate}
    ...
    \item
      Der Quotientenkörper~$K(X) \defined \Quot(K[X])$ der \defemph{Körper der rationalen Funktionen} über~$K$.
  \end{enumerate}
\end{example}
...
\begin{example}
  Der Polynomring~$K[X]$ ein Hauptidealring und es gilt~$K[X]^\times = K^\times = K \setminus \{0\}$.
...
\begin{definition}
  Es sei~$p$ ein Element von~$R$ mit~$p \neq 0$, und~$p$ sei keine Einheit.
  \begin{enumerate}
    \item
...
\begin{lemma}
  ...
  \begin{enumerate}
    ...
    \item
      Ein Element~$p$ ist genau dann irreduzibel, wenn~$\genideal{p} \neq 0, R$ gilt, und für jedes Hauptideal~$I$ in~$R$ mit~$\genideal{p} \subseteq I \subseteq R$ gilt bereits~$I = \genideal{p}$ oder~$I = R$ gilt.
  \end{enumerate}
\end{lemma}
...
\begin{proposition}
  ...
  \begin{enumerate}
    ...
    \item
      Ist~$R$ ein Hauptidealring, so sind die folgenden Bedingungen an~$p \neq 0$ äquivalent.
      \begin{equivlist}
        \item
      ...
      \end{equivlist}
      Inbesondere ist ein Ideal~$I \neq 0$ in~$R$ genau dann prim, wen es maximal ist.
      (Das Nullideal ist prim, aber nur genau dann maximal, wenn~$R$ bereits ein Körper ist.)
  \end{enumerate}
\end{proposition}
...
\begin{example}
  ...
  \begin{enumerate}
    \item
      ...
      Es gilt dabei~$\Integer / I \cong \Integer_p$ und somit
      \begin{align*}
        \text{$I$ ist prim}
        &\iff
       ...
       \end{align*}
     \item
       Das Ideal~$I = \genideal{Y}$ in~$K[X,Y]$ ist prim aber nicht nicht maximal.
...
\begin{proposition}
  Ist der Ring~$R$ faktoriell, so sind ist ein Element von~$R$ genau dann prim, wenn es irreduzibel ist.
\end{proposition}
...
\begin{corollary}
  Jeder euklidische Ring ist faktoriell.
\end{corollary}

\begin{corollary}
  Ist~$R$ ein Hauptidealring und ist~$p \in R$ irreduzibel, so ist~$R / \genideal{p}$ ein Körper.
\end{corollary}
...
\begin{example}
  ...
  \begin{enumerate}
    ...
    \item
      ...
      Ist ein Polynom~$f \in K[X]$ irreduzibel, so ist~$K[X] / \genideal{f}$ ein Körper.
  \end{enumerate}
\end{example}
...
\begin{example}
  ...
  \[
    ...
    \Natural \,.
  \]
  Wie in \cref{units in gaussian integers} erhalten wir aus der Multiplikativität der Norm, dass
  \[
    \Integer[\sqrt{-5}]^\times
    =
    \{ 1, -1 \} \,.
  ...
  \[
    ...
    N(z_2) = 6 \,.
  \]
  In jedem Fall gilt für eines der~$z_i$, dass~$N(z_i) = 1$, weshalb~$z_i$ eine Einheit in~$\Integer[\sqrt{-5}]$ ist.
  ...
  Wir betrachten nun für~$z = 6$ die beiden Zerlegungen
  \[
    z = 2 \cdot 3 \,,
    \quad
    z = ( 1 + i \sqrt{5} ) ( 1 - i \sqrt{5} ) \,.
  \]
  Dies sind zwei Zerlegungen von~$z$ in irreduzibel Element von~$\Integer[\sqrt{-5}]$.
...
\begin{example}
  ...
  \begin{enumerate}
    ...
    \item
      Ein Polynom~$f \in R[X]$ ist genau dann primitv, wenn für jede Einheit~$\varepsilon \in R^\times$ das Polynom~$\varepsilon f$ primitiv ist.
    \item
      Jedes Polynom in~$K[X]$ ist primitiv.
    \item
      Das Polynom~$2X + 4$ ist nicht primitiv in~$\Integer[X]$, aber primitiv in~$\Rational[X]$.
  \end{enumerate}
\end{example}

\begin{proposition}[Lemma von Gauß]
  Sind~$f, g \in R[X]$ zwei primitive Polynome, so ist auch ihr Produkt~$fg$ primitiv.
\end{proposition}
...
\begin{remark}
  \Cref{polynomial rings in multiple variables are ufd} gilt auch für allgemeiner für Polynomringe~$R[X_i \suchthat i \in I]$ in beliebig vielen Variablen.
...
\begin{proposition}[Eisenstein-Kriterium]
  ...
  \[
    ...
    p^2 \ndivides a_0 \,.
  \]
  Dann ist das Polynom~$f$ irreduzibel in~$\Quot(R)[X]$.
  Ist das Polynom zusätzlicherweise~$f$ primitiv, so ist es außerdem irreduzibel in~$R[X]$.
\end{proposition}
